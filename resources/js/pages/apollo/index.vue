<template>
    <div class="row">
        <div class="col-lg-10 m-auto">
            <card>
                <h2>Apollo Exam</h2>
                <p>The list of random strings are used. Starting from the center of the spiral to the tip, extracting alphanumric characters from the randoms.</p>
                <ol>
                    <li>{{random[0]}}</li>
                    <li>{{random[1]}}</li>
                    <li>{{random[2]}}</li>
                ...
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMin slice" viewBox="0 0 728 400">
                    ...
                    <textPath id="text" xlink:href="#s">{{finalString}}</textPath>
                    </text>  
                </svg> 
                </div>
    ...
    export default {
        ...
        data(){
            return{
                ...
                finalString: '',
                random: [],
                length: 25 //default by iteration
            }
        },
        async created() {
            await axios
                .get("/api/apollo-exam/create")
                .then((res) => {
                    console.log("Create")
                    console.log(res);
                })
              ...
              await  axios
                .get("/api/apollo-exam/view")
                .then((res) => {
                    console.log("View")
                    console.log(res);
                    console.log(res.data.breakdown.data);
                    this.breakdown = res.data.breakdown.data.values;
                    for(var i = 0; i < this.length; i++){
                        this.finalString += res.data.breakdown.data[i].values;
                        this.finalString += " "
                    }
                    
                    for(i = 0; i < 5; i++){
                        this.random.push(res.data.random.data[i].values);
                    }
                    console.log(this.random[0]);
